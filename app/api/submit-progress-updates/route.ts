...
import { createProgressUpdatesSubmission, ProgressUpdatesSubmission } from '@/lib/cosmos-rest';
...
// Sanitize input data for progress updates
function sanitizeData(data: any): Partial<ProgressUpdatesSubmission> {
  return {
    name: data.name?.trim(),
...
export async function POST(request: NextRequest) {
  try {
    console.log('API: Progress updates form submission started');
    ...
    let submission;
    try {
      submission = await createProgressUpdatesSubmission(submissionData);
    ...
    console.log('Progress updates form submission created:', {
      id: submission.id,
    ...
    return NextResponse.json({
      success: true,
      message: 'Progress updates form submitted successfully',
      submissionId: submission.id,
    });

  } catch (error) {
    console.error('Progress updates form submission error:', error);
...
export async function GET() {
  return NextResponse.json({
    status: 'Progress updates API is working',
