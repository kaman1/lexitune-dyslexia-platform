...
//   const key = (await question('Enter your Stripe Secret Key: ')).trim();
...
//   console.log('It should start with "sk_test_".');
...
//       if (!err.message?.includes('already uses that lookup key')) {
//         throw error;
//       }
//       console.log(chalk.yellow('Basic product already exists, skipping'));
//     }
...
//         throw error;
//       }
//       console.log(chalk.yellow('Standard product already exists, skipping'));
//     }

//     let enterpriseProduct: string | undefined;
//     try {
...
//       });
//       enterpriseProduct = enterprisePrice.product as string;
//     } catch (error: unknown) {
//       const err = error as { message?: string };
//       if (!err.message?.includes('already uses that lookup key')) {
//         throw error;
//       }
//       console.log(chalk.yellow('Enterprise product already exists, skipping'));
      
//       // Try to get the existing product ID
//       const prices = await stripe.prices.list({
//         lookup_keys: ['enterprise'],
//         limit: 1,
//       });
//       if (prices.data.length > 0) {
//         enterpriseProduct = prices.data[0].product as string;
//       }
//     }
...
//         await stripe.products.createFeature(enterpriseProduct, {
//           entitlement_feature: auditLogsFeature.id,
//         });
//       } catch (error: unknown) {
//         console.log(chalk.yellow('Could not add features to Enterprise product, it might already have them'));
//       }
//     }

//     console.log(chalk.green('Stripe test mode products and prices generation completed'));
//   } catch (error) {
...
//   const key = (await question('Enter your test WorkOS API Key: ')).trim();
...
// async function checkExistingConvexUrl(): Promise<string | null> {
//   try {
...
//       if (convexUrl) {
//         console.log(chalk.green(`Found existing Convex URL: ${convexUrl}`));
//         return convexUrl;
//       }
//     }
...
//     return null;
//   } catch (error) {
//     console.log(chalk.yellow('Could not check for existing Convex URL'));
//     return null;
//   }
// }
...
//   const skipSetup = await question('Convex already set up? (y/n): ');
//   if (skipSetup.toLowerCase() === 'y') {
//     console.log(chalk.green('Skipping Convex project creation'));
//     return;
//   }
...
//   // Check for existing Convex URL before setting up Convex
//   let convexUrl = await checkExistingConvexUrl();
...
//     const newConvexUrl = env.match(/NEXT_PUBLIC_CONVEX_URL=(.*)/)?.[1];
//     convexUrl = newConvexUrl || null;
//   }
  
//   if (!convexUrl) {
//     console.log(chalk.red('Could not find Convex URL. Webhook setup will be skipped.'));
//     console.log('\n🎉 Setup completed partially!');
